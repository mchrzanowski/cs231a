...
def _generate_fvs(matlab_src_dir, param_dir, vlfeat_dir, file1, file2):
    ...
    cmd = """cd %s; matlab -nosplash -nodesktop -nojvm -r "remote_create_fv('%s', '%s', '%s', '%s', '%s', '%s')" > /dev/null""" % (matlab_src_dir,
        file1, fv1_file, file2, fv2_file, param_dir, vlfeat_dir)
...
def decide(matlab_dir, param_dir, vlfeat_dir, file1, file2):
    try:
        fv1, fv2 = _generate_fvs(matlab_dir, param_dir, vlfeat_dir, file1, file2)
        global _W, _b
